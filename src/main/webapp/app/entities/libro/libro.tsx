...
import { getEntities, reset } from './libro.reducer';
...
export const Libro = () => {
  ...
  const links = useAppSelector(state => state.libro.links);
  ...
  return (
    <div>
      ...
      <div className="table-responsive">
        <InfiniteScroll
          dataLength={libroList ? libroList.length : 0}
          next={handleLoadMore}
        ...
        >
          {libroList && libroList.length > 0 ? (
              ...
              <thead>
                <tr>
                  ...
                  <th className="hand" onClick={sort('titolo')}>
                    Titolo <FontAwesomeIcon icon={getSortIconByFieldName('titolo')} />
                  </th>
                  <th className="hand" onClick={sort('prezzo')}>
                    Prezzo <FontAwesomeIcon icon={getSortIconByFieldName('prezzo')} />
                  </th>
                  <th>
                    Autore <FontAwesomeIcon icon="sort" />
                  </th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {libroList.map((libro, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>{libro.titolo}</td>
                    <td>{libro.prezzo}</td>
                    <td>{libro.autore ? <Link to={`/autore/${libro.autore.id}`}>{libro.autore.id}</Link> : ''}</td>
                    <td className="text-end">
                      <div className="btn-group flex-btn-group-container">
                        ...
                        <Button
                          onClick={() => (window.location.href = `/libro/${libro.id}/delete`)}
                          color="danger"
                          size="sm"
            ...
            !loading && <div className="alert alert-warning">No Libros found</div>
          )}
        </InfiniteScroll>
      </div>
    </div>
